...
form.addEventListener("submit", function (e) {
  ...
  const wordsValue = parseInt(words.value);
  const random = Math.floor(Math.random() * text.length);
  
  if (isNaN(wordsValue) === false && wordsValue > 0) {
    let wordArr = [];
    let word;
    let words = [];
  
    wordArr = wordSplit(text, wordArr);
    ...
    for (let i = 0; i < value; i++) {
      extractWords = [wordArr[i].slice(0, wordsValue)];
      paragraphs.push(extractWords);
    }

    let tempText1 = paragraphs.slice(0, paragraphs.length);
    for (let i = 0; i < paragraphs.length; i++) {
      word = tempText1[i].join(" ");
      word = word.replace(/,/g, " ");
      words.push(word);
    }
    console.log(words);
